...
`crates/unicity/payload` and the bounded `SealRegistry` execution kernel in
`crates/unicity/execution`. Neither is wired into an `EngineTypes`, node, RPC module, capability,
block builder or importer, so normal node operation and live execution semantics remain unchanged.
...
## M1 (bft-core #11): bounded SealRegistry execution kernel, inactive
...
Authentication and block context are explicit caller prerequisites. The crate does not authenticate
certificates or configuration, bind the supplied database cryptographically to the claimed parent,
execute ordinary transactions, reconcile headers or receipts, or activate build/import/replay or
Engine API paths. Its fixture provenance and bounded test inventory are recorded in
...
| Change | Kind |
| --- | --- |
| `Cargo.toml`: two workspace member lines, `crates/unicity/payload/` and `crates/unicity/execution/` | the only edits to an upstream source or manifest file |
| `Cargo.lock`: two added package entries, `reth-unicity-payload` and `reth-unicity-execution`, with no existing dependency version changed | generated by cargo for the two members |
| `crates/unicity/payload/` | inactive per-payload commitment provision |
| `crates/unicity/execution/` | inactive bounded registry execution kernel and its fixtures |
| `UNICITY.md` | this record |

No upstream crate source file or existing dependency version is changed. Check:
...
The first command must show only two added lines in `Cargo.toml` and the two generated package
entries in `Cargo.lock`; the second provides the exact auditable inventory.
