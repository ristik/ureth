...
| `.github/scripts/check_wasm.sh` | excludes the payload wrapper beside its native transaction-pool dependencies |
| `UNICITY.md` | this record |

No upstream runtime crate source file or existing dependency version is changed. Check:
...
The first command must match the inventory above; the second provides the exact workspace and lock
changes.
